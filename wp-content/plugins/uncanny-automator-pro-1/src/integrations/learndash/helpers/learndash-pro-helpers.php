...
class Learndash_Pro_Helpers extends Learndash_Helpers {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_select_lessontopic_from_course_LD_SUBMITASSIGNMENT', array(
			$this,
	 ...
	 * @return mixed
	 */
	public function all_ld_certificates( $label = null, $option_code = 'LDCERTIFICATES', $args = [] ) {
		global $uncanny_automator;
		if ( ! $label ) {
	...
	public function add_all_groups_option( $options ) {
		...
		if ( 'LDREMOVEGROUP' !== $options['option_code'] ) {
			return $options;
		}
