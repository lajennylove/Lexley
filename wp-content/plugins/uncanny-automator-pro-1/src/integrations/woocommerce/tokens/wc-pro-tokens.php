...
class Wc_Pro_Tokens extends Wc_Tokens {
	...
	public function __construct() {
		...
		add_action( 'uap_wc_trigger_save_meta', [ $this, 'uap_wc_trigger_save_meta_func' ], 40, 4 );
		add_action( 'uap_wc_order_item_meta', array( $this, 'uap_wc_order_item_meta_func' ), 40, 4 );
		add_action( 'uap_wc_trigger_save_product_meta', [ $this, 'uap_wc_trigger_save_product_meta_func' ], 40, 4 );

		add_filter( 'automator_maybe_trigger_wc_wooprodcat_tokens', [
			$this,
			'wc_wooprodcat_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_trigger_wc_woovariproduct_tokens', [
			$this,
			'wc_wooprodcat_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_trigger_wc_wooprodtag_tokens', [
			$this,
			'wc_wooprodcat_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_trigger_wc_wcprodreview_tokens', [
			$this,
		...
		//Adding WC tokens
		add_filter( 'automator_maybe_trigger_wc_wcshipstationproductshipped_tokens', [
			$this,
			'wc_order_possible_tokens',
		], 20, 2 );
		add_filter( 'automator_maybe_trigger_wc_wcshipstationordertotalshipped_tokens', [
			$this,
			'wc_order_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_trigger_wc_woopaymentgateway_tokens', [
			$this,
			'wc_wooprodcat_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_trigger_wc_anonorderitemcreated_tokens', [
			$this,
			'wc_wooorderitemadded_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_trigger_wc_addedtocart_tokens', [
			$this,
			'wc_addedtocart_possible_tokens',
		], 20, 2 );

		add_filter( 'automator_maybe_parse_token', [ $this, 'wc_addedtocart_tokens_pro' ], 26, 6 );

		add_filter( 'automator_maybe_parse_token', [ $this, 'wc_ordertotal_tokens_pro' ], 26, 6 );
		add_filter( 'automator_maybe_parse_token', [ $this, 'parse_subscription_tokens_pro' ], 36, 6 );
	}
	 ...
	 * @return array
	 */
	public function wc_wcprodreview_possible_tokens( $tokens = [], $args = [] ) {
	 ...
	 * @return array
	 */
	public function wc_wooprodcat_possible_tokens( $tokens = [], $args = [] ) {
		if ( ! automator_pro_do_identify_tokens() ) {
			return $tokens;
		}
	...
	public function wc_addedtocart_possible_tokens( $tokens = array(), $args = array() ) {
		...
		$fields       = array(
			...
			array(
				...
				'tokenName'       => __( 'Quantity', 'uncanny-automator-pro' ),
				'tokenType'       => 'text',
				'tokenIdentifier' => $trigger_meta,
			),
			array(
